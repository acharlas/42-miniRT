...
/*   Updated: 2019/12/03 18:11:27 by acharlas         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		scene_intersect(const vect3f *orig, const vect3f *dir, const t_list *listobj, vect3f *hit, vect3f *n, t_material *material)
{
	...
	while (listobj)
	{
		...
		if(listobj->name == 'p')
		{
			if(SQUARE->ray_intersect(orig, dir, &dist_i, *SQUARE) && dist_i < cylinder_dist && dist_i < square_dist && dist_i < spheres_dist)
			{
				square_dist = dist_i;
				*hit = v_plus(*orig, v_mult(*dir, dist_i));
				*n = normalize(SQUARE->rot);
				*material = SQUARE->material;
			}
		}
		if(listobj->name == 's')
		{
			if (SPHERE->ray_intersect(orig, dir, &dist_i, *SPHERE) && dist_i < cylinder_dist && dist_i < square_dist && dist_i < spheres_dist)
			{
				spheres_dist = dist_i;
				*hit = v_plus(*orig, v_mult(*dir, dist_i));
...
int		main(void)
{
	...
	cylinder->pos = c_vect3f(0, 5, -10);
	cylinder->h = 2;
	cylinder->material = ivoire;
	cylinder->r = 3;
	cylinder->rot = c_vect3f(0, 1, 0);
